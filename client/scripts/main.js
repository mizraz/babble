...
window.onload = function () {
    ...
    if (localStorage.getItem('babble') === null) {
     ...
     localStorage.setItem('babble', JSON.stringify(Babble.localStorageVar));
      ...
      try {
        ...
        console.log("curUserInfo" + curUserInfo.name);
        Babble.register(curUserInfo);
      }
      catch (e) {
        console.log("error reading babble local storage");
    ...
    Babble.btn.addEventListener('click', function(){
      ...
      //console.log('CLICKED! client messageText: ' + messageText +'\n');
      var msgJson = {
        name: Babble.name,
        email: Babble.email,        
        message: messageText,
 ...
 Babble = {
    ...
    register: function (userInfo) {
        ...
        var curUserInfo = {name: curName, email: curEmail};
      ...
      var jsonEmail = {email: Babble.email};
      xmlhttpRegister.send(JSON.stringify(jsonEmail));
      Babble.getMessages(0, function (arr) {console.log(
        "in callback of getMessages arr= " + arr)});
        ...
        xmlhttpStats.onreadystatechange = function (response) {
          ...
          if (xmlhttpStats.readyState === DONE) {
            if (xmlhttpStats.status === OK) {
              ...
              Babble.getStats(function(response){
                  if (Babble.xmlhttpStats.readyState === 4 && Babble.xmlhttpStats.status === 200){
                    console.log('befor drawStats: responseText:' + Babble.xmlhttpStats.responseText);
                    drawStats(Babble.xmlhttpStats.responseText);
                  }
              });     
            }
            ...
            } else if (xmlhttpStats.status == 404)  {console.log("response of  stats is error 404" );}
          }
      };
      xmlhttpStats.send();
    },
        ...
        lc.onload = function (response) {
          ...
          if (lc.readyState == DONE) {
            if (lc.status == OK) {
                ...
                callback(myArr);
                drawMessage(myArr);   
               Babble.getMessages(Babble.counter, function (arr) {console.log("in callback of getMessages arr= " + arr)});
           }
           else if (lc.status == 204) {
            Babble.getMessages(Babble.counter, function (arr) {console.log(
              "in callback of getMessages arr= " + arr)});
          }
        }     
      };
        ...
        xmlhttpDel.onload = function(response) {
          ...
          if (xmlhttpDel.readyState === DONE) {
            if (xmlhttpDel.status === OK) {
              ...
              console.log("message id not exist so cann't del it");
          } 
        }
      xmlhttpDel.send();
    }
...
drawMessage = function (myArr) {
  console.log("myArr " + myArr + '\nwith length' + myArr.length);
  for (i = 0; i < myArr.length; ++i) {
    console.log("drawMessage iteratoion: "+i);
    var curMessage = myArr[i];
    console.log("myArr[i] " + JSON.stringify(curMessage));
